...
mountusb(){ \
    chosen=$(echo "$usbdrives" | rofi -theme themes/distrotube.rasi -dmenu -show run -lines 5 -opacity "85" -bw 0 -width 30 -padding 20 -i -p "Mount which drive?" | awk '{print $1}')
...
umountusb(){ \
    chosen=$(echo "$mounteddrives" | rofi -theme themes/distrotube.rasi -dmenu -show run -lines 5 -opacity "85" -bw 0 -width 30 -padding 20 -i -p "Unmount which drive?" | awk '{print $1}')
...
umountall(){ \
    for chosen in $(echo $(lsblk -rpo "name,type,size,mountpoint" | grep -v $USB | awk '$2=="part"&&$4!=""{printf "%s\n",$1}')); do
...
usbdrives="$(lsblk -rpo "name,type,size,mountpoint" | grep -v $USB | awk '$2=="part"&&$4==""{printf "%s (%s)\n",$1,$3}')"
mounteddrives="$(lsblk -rpo "name,type,size,mountpoint" | grep -v $USB | awk '$2=="part"&&$4!=""{printf "%s (%s)\n",$1,$3}')"
